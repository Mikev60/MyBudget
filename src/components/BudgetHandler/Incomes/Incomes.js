...
const onClickAddHandler = () => {
    const type = {type: inputValue, userId: props.userId}
    axios.post('/typesIn.json?auth='+props.token, type)
    .then(response => {
...
const onCLickDeleteHandler = (id) => {
    ...
    axios.delete('/typesIn/'+id+'.json?auth='+props.token)
    .then(response => {
...
const modifyHandler = () => {
    ...
    for(let i = 0; i < oldInputs.length ; i++) {
        ...
        axios.put('/typesIn/'+oldInputs[i].id+'.json?auth=' +props.token, objectToUpdate)
        .then(response => {
...
useEffect(() => {
    axios.get('/typesIn.json?auth=' +props.token+'&orderBy="userId"&equalTo="'+props.userId+'"')
    .then(response => {
        let types = []
